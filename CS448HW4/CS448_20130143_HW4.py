...
def xor(a, b):
    x = ('0' * 32 + hex(int(btoh(a), base=16) ^ int(btoh(b), base=16))[2:])[-32:]
...
print(btoh(ciphertext))
print(btoh(aes.decrypt_block(ciphertext)))
'''
...
CTR = bytearray.fromhex(CTR)

PT1 = PT[:32]
PT2 = PT[32:64]
PT3 = PT[64:96]
PT4 = PT[96:]
PT1  = bytearray.fromhex(PT1)
PT2  = bytearray.fromhex(PT2)
PT3  = bytearray.fromhex(PT3)
PT4  = bytearray.fromhex(PT4)
PTB = [PT1, PT2, PT3, PT4]
...
for pt in PTB:
    ...
    print("PT  = %s" % btoh(pt))
    print("IN  = %s" % btoh(pt))
    out = aes.encrypt_block(pt)
    print("OUT = %s" % btoh(out))
    print("CT  = %s" % btoh(out))
    idx = idx + 1
print("\n")
...
for pt in PTB:
    ...
    ct = xor(pt, out)
    print("CT  = %s" % btoh(ct))
    chain = add(chain, bytearray.fromhex('01'))
    idx = idx + 1
print("\n")